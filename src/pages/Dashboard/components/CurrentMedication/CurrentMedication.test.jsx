...
describe("CurrentMedication", () => {
  ...
  it("updates taken counts when the Taken button is pressed", async () => {
    ...
    const medicationCell = await screen.findByText("Ibuprofen");
    const medicationRow = medicationCell.closest("tr");
    expect(medicationRow).not.toBeNull();
    expect(within(medicationRow).getByText("1")).toBeInTheDocument();
    ...
    expect(await within(medicationRow).findByText("2")).toBeInTheDocument();
  });
